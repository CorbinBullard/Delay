...
router.get("/:itemId", async (req, res) => {
  const data = await Item.findByPk(req.params.itemId, {
    include: [
      { model: ProductReview, include: { model: User } },
      { model: ItemImage },
      { model: User },
    ],
  });
...
router.post("/", singleMulterUpload("image"), requireAuth, async (req, res) => {
  const { name, brand, price, description, instrumentType, year, condition } =
    req.body;
  const { user } = req;
...
router.post("/:itemId/reviews", requireAuth, async (req, res) => {
  ...
  const userReviews = await item.getProductReviews({
    where: { userId: user.id },
  });
  if (userReviews.length)
    return res
      .status(403)
      .json({ message: "User already has a review for this item" });
